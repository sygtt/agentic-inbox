...
- Opening a message records its ID in the URL so browser Back returns to the same loaded list and preserves its local filters and scroll position. Previous/next controls follow the loaded list order without adding history entries; opening another message marks it read and resets detail scrolling to the top. A successful archive selects the next lower message when available, while a failed archive leaves the detail open.
